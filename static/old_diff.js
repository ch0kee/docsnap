...
var JsDiff = (function() {
  ...
  var Diff = function() {
    ...
    //TODO: hibakezeles
    pushComponents : function(components, text, first, count, added, removed) {
        //exceptional cases shortcircuit
        if (count == 0)
            return;
        if (count == 1)
            pushComponent(components, text[first], added, removed);
        ///

        if (components.length == 0) {
            components.push({value: text.slice(first, first+count), added: added, removed: removed });
        }
        var last = components[components.length-1];
        for(var i = first; i < first+count; ++i) {
            var value = text.slice(first, first+count);
            if (last.added === added && last.removed === removed) {
                last.value += value;
            } else {
                last = {value: value, added: added, removed: removed }
      ...
      extractCommon: function(basePath, newString, oldString, diagonalPath) {
        ...
        //consume snake
        var old_newPos = newPos;
        while (newPos+1 < newLen && oldPos+1 < oldLen && this.equals(newString[newPos+1], oldString[oldPos+1])) {
          newPos++;
          oldPos++;
        }
        this.pushComponents(basePath.components, newString, newPos, newPos - old_newPos, undefined, undefined);
